...
class NetCDFStubGenerator:
    ...
    def create_sample_stub(cls, output_path: Path) -> None:
        with Dataset(output_path, mode="w", format="NETCDF4") as dataset:
            ...
            precipitation[:, :, :] = cls.rng.uniform(size=(100, 168, 162))
    ...
    def create_sample_variables(cls) -> dict[str, ParsedVariable]:
        return {
            ...
            "time": ParsedVariable(
                "days since 2020-01-01T00:00:00", np.ma.masked_array(np.arange(0, 100, 1))),
            "pr": ParsedVariable(
                "mm", np.ma.masked_array(cls.rng.uniform(size=(100, 168, 162)))),
        }
